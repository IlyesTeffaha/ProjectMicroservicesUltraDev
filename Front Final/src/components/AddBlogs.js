...
import { ProfileCardWidget,ChoosePhotoWidgetblog } from "../components/Widgets";
...
const addBlog=()=>{
  const data={title:title,description:desc,image:authState.blogimage,author:authState.username};
  const names=[]
  // const data={}
...
console.log(data)
  ...
  axios.post("http://localhost:8090/blog/addblog",data,config).then((response)=>{
//     if(response.data.message){
//       setErrorexist(true)

//       setErrmsg(response.data.message)
 setTimeout(() => { history.push(Routes.BlogsList.path)}, 1000);
    ...
    <Card border="light"  className="bg-white shadow-sm mb-4">
      <Card.Body>
        ...
        <Form>
          ...
          <ChoosePhotoWidgetblog  title="Select a photo"/>
