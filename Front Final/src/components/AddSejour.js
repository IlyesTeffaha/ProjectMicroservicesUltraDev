...
import '../pages/css/SejourList.css'
import { Routes } from "../routes";
import { useHistory } from "react-router-dom";
import { faAngleDown, faAngleUp, faChartArea, faChartBar, faChartLine, faFlagUsa, faFolderOpen, faGlobeEurope, faPaperclip, faUserPlus } from '@fortawesome/free-solid-svg-icons';
import { ProfileCardWidget,ChoosePhotoWidgetsejour } from "../components/Widgets";
import 'react-quill/dist/quill.snow.css'; 
import 'react-quill/dist/quill.bubble.css'; 
import EditorToolbar, { modules, formats } from "./editortoolbar";
import ReactQuill from 'react-quill';

export const AddSejour = () => {
...
const addsejour=()=>{
  const data={titre:title,maxCap:maxcap,image:authState.sejourimage,description:desc,datedepart:datebegin,dateArrive:dateend}
...
console.log(data)
  ...
  axios.post("http://localhost:8082/sejour/addsejour",data,config).then((response)=>{
//     if(response.data.message){
//       setErrorexist(true)
setTimeout(() => { history.push(Routes.SejourList.path)}, 1000);
...
<Form.Group id="firstName" >
              <Form.Label>Max capacity</Form.Label>
                ...
                <Form.Control placeholder="Enter Title" required type="number" />
                </InputGroup>
              </Form.Group>
            </Col>
            
          <ChoosePhotoWidgetsejour title="Select a photo"/>
          ...
          <Form.Group id="firstName" >
              <Form.Label>Start date</Form.Label>
                <InputGroup  onChange={(e)=>{setDatebegin(e.target.value)}}>
                
                <Form.Control placeholder="Enter date" required type="date" />
                </InputGroup>
              </Form.Group>

              <Form.Group id="firstName" >
              <Form.Label>End date</Form.Label>
                <InputGroup  onChange={(e)=>{setDateend(e.target.value)}}>
                
                <Form.Control placeholder="Enter Title" required type="date" />
                </InputGroup>
              </Form.Group>
  ...
  <option value={items.name} key={index}>{items.name} </option>
        ...
        <br/>
          ...
          <div className="mt-3 mb-3">
            <Button variant="primary" onClick={addsejour}>Add Sejour </Button>
          </div>
          {errorexist?(
                        <>
