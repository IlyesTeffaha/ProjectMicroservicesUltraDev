...
import { ProfileCardWidget,ChoosePhotoWidgetplaces } from "../components/Widgets";
...
const addBlog=()=>{
  const data={placeTitle:title,type:chosentype,image:authState.restaurantimage,description:desc,country:str[0],city:chosencity,state:idn[0]};
  // const data={}
...
console.log(data)
  ...
  axios.post("http://localhost:8086/place/addplace",data,config).then((response)=>{
//     if(response.data.message){
//       setErrorexist(true)
setTimeout(() => { history.push(Routes.PlacesList.path)}, 1000);
...
<Form.Group id="firstName" >
              ...
              <select className="form-select" aria-label="Default select example" onChange={(e)=>setChosentype(e.target.value)}>
  <option defaultValue="Restaurant">Choose a type</option>
  <option value="Restaurant">Restaurant</option>
  <option value="Touristic place">Touristic place</option>
</select>
              </Form.Group>
            </Col>
            
          <ChoosePhotoWidgetplaces title="Select a photo"/>
  ...
  <option value={items.name} key={index}>{items.name} </option>
        ...
        <div className=""><ReactQuill
          ...
          <div className="mt-3 mb-3">
            <Button variant="primary" onClick={addBlog}>Add Places </Button>
          </div>
          {errorexist?(
                        <>
