...
import '../pages/css/SingleBlog.css'
...
function SingleBlog() {
    ...
    useEffect(()=>{
        axios.get(`http://localhost:8090/blog/getblog/${id}`,{headers:{accessToken:localStorage.getItem("accessToken")},
    }).then((response)=>{
      ...
      const handleUpdate = async () => {
        try {
          await axios.put(`http://localhost:8090/blog/updateblog/${id}`, {
            // name:data.name,
            title:title,
            description:desc,
          });
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:8090/blog/delete/${id}`, {
        // data: { name: data.name },
      });
      console.log("deleted")
   history.push(Routes.BlogsList.path)
...
//                                 <h3 className ="dark-color">{blogdata.title}</h3>
//                                 {/* <h6 className ="theme-color lead">A Lead UX &amp; UI designer based in Canada</h6>
//                                 <p>I <mark>design and develop</mark> services for customers of all sizes, specializing in creating stylish, modern websites, web services and online stores. My passion is to design digital blog experiences through the bold interface and meaningful interactions.</p> */}
...
//                                             <p>{new Date(blogdata.creationtime).toDateString()}</p>
//                                         </div>
...
//                                         <div className ="media">
//                                             <label>Phone</label>
//                                             <p>{blogdata.PhoneNumber}</p>
//                                         </div>
//                                         <div className ="media">
//                                             <label>Current Status</label>
//                                             <p>{blogdata.currentstatus}</p>
//                                         </div> */}
      ...
      <div className="singlePostWrapper">
        ...
        {updateMode ? (
          ...
          <h1 className="singlePostTitle">
            {authState.username==post.author&& (
        ...
        <div className="singlePostInfo">
          <span className="singlePostAuthor">
            ...
            <Link to={`/apps/blog/blog-home?user=${post.emailvalue}`} >
              <font className="linkc"> {post.author}</font>
