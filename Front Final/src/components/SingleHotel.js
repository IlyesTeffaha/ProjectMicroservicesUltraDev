...
import '../pages/css/SingleHotel.css'
...
import { faWarehouse,faHandshake,faHotel,faMapMarked ,faMailBulk} from '@fortawesome/free-solid-svg-icons'
...
function SingleHotel() {
    ...
    console.log(desc)
    useEffect(()=>{
        axios.get(`http://localhost:8083/hotel/gethotel/${id}`,{headers:{accessToken:localStorage.getItem("accessToken")},
    }).then((response)=>{
        setPost(response.data)
            console.log(response.data)
            setDescription(response.data.description)
            setDesc(response.data.description);
            setTitle(response.data.hotelName)
        })
    },[id]);
      ...
      const handleUpdate = async () => {
        try {
          await axios.put(`http://localhost:8083/hotel/updatehotel/${id}`, {
            // name:data.name,
            hotelName:title,
            description:desc,
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:8083/hotel/delete/${id}`, {
        // data: { name: data.name },
      });
      console.log("deleted")
   history.push(Routes.HotelList.path)
...
//                                 <h3 className ="dark-color">{hoteldata.title}</h3>
//                                 {/* <h6 className ="theme-color lead">A Lead UX &amp; UI designer based in Canada</h6>
//                                 <p>I <mark>design and develop</mark> services for customers of all sizes, specializing in creating stylish, modern websites, web services and online stores. My passion is to design digital hotel experiences through the bold interface and meaningful interactions.</p> */}
...
//                                             <p>{new Date(hoteldata.creationtime).toDateString()}</p>
//                                         </div>
...
//                                         <div className ="media">
//                                             <label>Phone</label>
//                                             <p>{hoteldata.PhoneNumber}</p>
//                                         </div>
//                                         <div className ="media">
//                                             <label>Current Status</label>
//                                             <p>{hoteldata.currentstatus}</p>
//                                         </div> */}
...
//                                 <img src={hoteldata.image} title="" alt=""/>
//                             </div>
//                         </div>
//                     </div>
